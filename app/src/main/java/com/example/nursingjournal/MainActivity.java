...
public class MainActivity extends AppCompatActivity {
    static final int VISIT_CHECK = 1;
    SharedPreferences isVisited;
...
//        previewFrame.addView(cameraView);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == VISIT_CHECK && resultCode == RESULT_OK){
            editor = isVisited.edit();
            boolean visited = data.getBooleanExtra("Visit",false);
            childId = data.getIntExtra("childId",1);
    ...
    protected void onResume() {
        super.onResume();
        //처음 실행이 아닐 시
        isVisited = getSharedPreferences("Visit",Activity.MODE_PRIVATE);
        if (isVisited == null && !isVisited.contains("Visit")){
            //처음 실행 시
            Intent registerIntent = new Intent(getApplicationContext(), RegisterActivity.class);
            startActivityForResult(registerIntent, VISIT_CHECK);
        }
    }
